...
public class OrnsteinScript : EnemyBase
{
    ...
    [SerializeField]
    private float QuickAttackPauseTimeUntilAim, QuickAttackPauseTimeAfterAim;
    [SerializeField]
    ...
    private IEnumerator AimTimer() {
        ...
        while (true) {
            ...
            if (shotsSinceQuickShot == 0) {
                yield return new WaitForSeconds(aimRate+ QuickAttackPauseTimeUntilAim + QuickAttackPauseTimeAfterAim + rand);
            } else {
                yield return new WaitForSeconds(aimRate + rand);
            }
